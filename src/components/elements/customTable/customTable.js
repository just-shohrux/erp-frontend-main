...
export const Row = ({children, className='', ...props}) => (
    <div className={`tr ${className}`} {...props}>{children}</div>
)

export const Column = ({children, className='', reff=()=>{}, ...props}) => (
    <div className={`td ${className}`} ref={reff} {...props}>{children}</div>
)

export const Tbody = ({children, className='', reff=getRefValueBody, ...props}) => (
    <div className={`tbody ${className}`} ref={reff}>{children}</div>
)

export const Thead = ({children, className='', reff=getRefValueBody, ...props}) => (
    <div className={`thead ${className}`} ref={reff}>{children}</div>
)
...
export default function CustomTable({ children, columns=[], data=[], ...props}) {
    ...
    return (
        <DefaultCustomTableStyled className="table" {...{...props, maxWidth}} >
            {isArray(columns) && columns.length > 0 && <Thead reff={data.length ? null : getRefValueBody}>
                <Row>
                    {columns.map(({title, dataIndex}, index) => <Column 
                        reff={data.length ? getRefValue : null} 
            ...
            {isArray(data) && data.length > 0 && <Tbody reff={data.length ? null : getRefValueBody}>
            {isArray(data) && data.length > 0 && data.map((value, index) => (
                <Row key={value.key}>
                    {isArray(columns) && columns.length && columns.map(({dataIndex, key}) => <Column 
                        reff={data.length ? getRefValue : null} 
                        key={key} 
                        data-index={dataIndex} 
                        >  {value[dataIndex]} </Column>
                    )}
                </Row>
            ))}
            </Tbody>}
